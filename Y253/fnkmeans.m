...
out2 = col2im(C_filter, [N N], size(I_pad), 'sliding');
 ...
 figure(7)
 imshow(out2,[]), title('Gaussian filtered image');
 imwrite(out2,'Y253_Gaussian.jpg');
 %%
...
figure(8)
imshow(finalImage, []);
%imwrite(finalImage,'Y1_Skull.jpg');
%%
%//K MEANS ALGORITHM
...
subplot(2,2,4);
 ...
 imtool(imresult,[]);
 imwrite(imresult,'Y253_Kmeans.jpg');
...
nroi_image=simg; 
imwrite(nroi_image,'Y253_Kmeans_NROI.jpg');
logimg=imsubtract(orgimg,simg);
roi_image=logimg;
imwrite(roi_image,'Y253_Kmeans_ROI.jpg');
 subplot(2,1,2);
